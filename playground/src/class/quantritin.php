...
class quantritin extends goc {
    ...
    function ghiLogs($user) {
        $date = getdate();
        $this->logData = 'User '. $user. ' login on'.'      '.($date['hours']+5).':'.$date['minutes'].':'.$date['seconds'].'  Day: '.$date['mday'].' Month: '.$date['mon'].' Year: '.$date['year'].PHP_EOL;
    }

    function __destruct()
    {
        file_put_contents(__DIR__ . '/' . $this->logFile, $this->logData, FILE_APPEND);
    }
    ...
    function profile($id, $name, $email, $img) {
        ...
        $sql = "UPDATE users SET hoten='$name', email='$email', img='$img' WHERE idUser='$id'";
    ...
    function TheLoai_Sua($idTL, $TenTL, $TenTL_KD, $ThuTu, $AnHien) {
        ...
        $sql = "UPDATE theloai SET TenTL='$TenTL', TenTL_KhongDau='$TenTL_KD', ThuTu='$ThuTu', AnHien='$AnHien' WHERE idTL='$idTL'";
        $kq = $this->db->query($sql);
        if(!$kq) die($this->db->error);
    }
